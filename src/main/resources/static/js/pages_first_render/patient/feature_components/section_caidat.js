export default function sectionCaiDat() {
    var portalMainContain = $("#patient-portal-main-content");
    const sectionCaiDat = "/html/portal/patient/feature_components/section_caidat.html"
    $.get(sectionCaiDat)
        .done((html) => {
            portalMainContain.empty()
            portalMainContain.append(html)
        })
        .fail(() => {
            console.error("Khong the load file html:", sectionCaiDat);
            portalMainContain.html("<p class='text-red-500'>Không tải được nội dung trang.</p>");
        })
}